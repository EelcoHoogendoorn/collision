...
class VertexGridHash {
    ...
    */
    typedef int32 index_type;       // needs be no bigger than number of vertices
...
public:
	const ndarray<1, const vector_type> position;    // vertex positions
	const index_type                    n_vertices;  // number of vertices
	const vector_type_scalar            lengthscale; // size of a virtual voxel, which equals the maximum interaction range of a vertex-vertex pair query
...
protected:
	...
	const index_type n_buckets;	                                // number of buckets
	const HashMap<cell_type, index_type> bucket_from_cell;  // maps cell coordinates to bucket index
...
public:
	...
	explicit VertexGridHash(const ndarray<2, vector_type_scalar> position, const vector_type_scalar lengthscale):
		...
		bucket_from_cell(       // create a map to invert the cell_from_bucket function
		    irange(0, n_buckets) | transformed([&](const index_type i){return cell_from_bucket(i);}),
		    irange(0, n_buckets)
		)
	{
...
protected:
	...
	inline cell_type cell_from_bucket(const index_type b) const {
	...
	const extents_type init_extents() const
	{
		...
		for (const vector_type p: position)
		{
	...
	const cell_type init_size() const {
	...
	const ndarray<1, const cell_type> init_cells() const
	{
	    ...
	    ndarray<2, cell_type_scalar> cidxbase({n_vertices, NDim});
	    ndarray<1, cell_type> cidx = cidxbase.view<cell_type>();
		for (auto v: irange(0, n_vertices))
	...
	const ndarray<1, index_type> init_indices() const
	{
	    ...
	    ndarray<1, index_type> _indices({n_vertices});
		boost::copy(irange(0, n_vertices), _indices.begin());
		boost::sort(
	        _indices,
			[&](auto l, auto r) {
				auto cl = cell_id[l];
	...
	const index_type init_pivots()
	{
		index_type np = 0;		//number of pivots
		auto add_pivot = [&](index_type b) {pivots[np] = b; np += 1;};

        auto res = indices
                    | transformed([&](index_type i){return cell_id[i];})
                    | indexed(1)
...
protected:
	auto indices_from_bucket(const index_type b) const {
	...
	auto vertices_from_cell(const cell_type& cell) const {
		return indices_from_cell(cell)
		        | transformed([&](const index_type i){return indices[i];});
    }
...
public:
	...
	void for_each_vertex_in_cell(const cell_type& cell, const F& body) const {
		for (auto i: indices_from_cell(cell))
			body(indices[i]);
	}
	...
	{
		const auto in_box = [&](const index_type v)
		{
			const vector_type vp = position[v];
			return !((vp < gmin).any() || (vp > gmax).any());
		};
