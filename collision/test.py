...
def test_box_grid():
    ...
    n = 4
    ndim = 2
    points = (np.random.rand(n, ndim) * (np.arange(ndim)+1)).astype(np.float32)
    scale = 0.01
    bounds = np.array([np.zeros(ndim), np.arange(ndim)+1])

    boxes = points[:, None, :]
    boxes = np.concatenate([boxes, boxes + scale*2], axis=1)
